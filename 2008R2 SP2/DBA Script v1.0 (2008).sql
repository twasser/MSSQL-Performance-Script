...
@infoSEC VARCHAR(5), @infoFRAG VARCHAR(5), @infoSTAT VARCHAR(5), @Inventory VARCHAR(5), @Version as VARCHAR(128), @VersionMajor DECIMAL(10,2), @VersionMinor DECIMAL(10,2)
...
IF @VersionMajor = '10.50' AND @VersionMinor NOT IN ('4000', '4260', '4263' ,'4266' ,'4270' ,'4276' ,'4279' ,'4286' ,'4290','4295' ,'4297' ,'4302' ,'4305' ,'4319' ,'6000' ) BEGIN
...
IF @VersionMajor IN ('11.00', '12.00') BEGIN
	...
	PRINT 'We have detected that you are on a newer version of MSSQL than this script is designed for. Please use the version appropriate for your version.'
	SELECT @@VERSION
	RAISERROR ('WARNING:  Version of SQL Server is not compatible with this script.', 20, 1) WITH LOG;
END
...
IF @infoBAK = 'TRUE' BEGIN
	...
	*/	
	...
	SELECT 
		...
		,RTRIM(CAST (BF.physical_name AS VARCHAR(50))) AS [Physical Name]
...
IF @infoSTAT = 'TRUE' or @Inventory = 'TRUE' BEGIN
   ...
   PRINT '##########  Last Statistice Gather Date #############';
	...
	EXEC ('USE ' + @DB + '
	SELECT TOP 50 RTRIM(CAST(o.name AS VARCHAR(50))) AS [Object Name], 
		o.[object_id], 
		RTRIM(CAST(o.type_desc AS VARCHAR)) AS [Type Desc],
		RTRIM(CAST(s.name AS VARCHAR(50))) AS [Statistics Name], 
